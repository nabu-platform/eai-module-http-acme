		...
		try {
			...
			if (artifact.getConfig().isEnabled() && artifact.getRepository().getServiceRunner() instanceof ClusteredServer) {
				for (VirtualHostArtifact virtualHost : hosts) {
					...
					if (virtualHost.getConfig().getKeyAlias() != null && virtualHost.getServer().getConfig().getKeystore() != null) {
						...
						if (keyAlias == null) {
							...
							PrivateKey privateKey = keystore.getKeyStore().getPrivateKey(keyAlias);
							if (privateKey == null) {
