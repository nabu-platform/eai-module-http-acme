...
public class Services {
	...
	public static VerificationResult verifyCertificate(AcmeArtifact artifact, boolean staging, long timeout, boolean persist, boolean force) {
		...
		try {
			...
			if (artifact.getConfig().getVirtualHosts() != null) {
				hosts.addAll(artifact.getConfig().getVirtualHosts());
			}
			if (artifact.getConfig().getServer() != null) {
				// dynamically fetch all hosts that are linked to this server
				for (VirtualHostArtifact host : artifact.getRepository().getArtifacts(VirtualHostArtifact.class)) {
					if (artifact.getConfig().getServer().equals(host.getServer())) {
						hosts.add(host);
					}
				}
			}
			
			if (artifact.getConfig().isEnabled() && artifact.getRepository().getServiceRunner() instanceof ClusteredServer) {
				for (VirtualHostArtifact virtualHost : hosts) {
					try {
						
						// we don't proceed if the target server is off
						if (virtualHost.getServer() == null || !virtualHost.getServer().getConfig().isEnabled()) {
							continue;
						}
						
						// we are only interested in hosts that have an alias linked to a server that has a keystore
						if (virtualHost.getServer().getConfig().getKeystore() != null) {
							...
							String acmeAlias = "acme2-" + virtualHost.getId();
							
							String keyAlias = virtualHost.getConfig().getKeyAlias();
							if (keyAlias == null) {
								keyAlias = virtualHost.getServer().getConfig().getDefaultAlias();
							}
							// the virtual host has no explicit alias, we use the virtual host id as alias
							if (keyAlias == null) {
								keyAlias = virtualHost.getId();
							}
							
							// we must check that a key exists, otherwise we must create it
							if (keystore.getKeyStore().getPrivateKey(keyAlias) == null) {
								// if it is not a dynamic keystore, we just skip it, this is actually the "old" behavior where you _had_ to provide a key alias in the host for this to work
								if (!keystore.isDynamic()) {
									continue;
								}
								KeyPair ownerKeyPair = SecurityUtils.generateKeyPair(KeyPairType.RSA, 4096);
								X500Principal principal = SecurityUtils.createX500Principal(virtualHost.getConfig().getHost(), "generated", "generated", "generated", "generated","generated");
								...
								keystore.getKeyStore().set(keyAlias, ownerKeyPair.getPrivate(), new X509Certificate[] {selfSignedCert}, UUID.randomUUID().toString().replace("-", ""));
							}
							...
							if (refresh) {
								...
								try {
									...
									Object object = map.get(virtualHost.getId() + "-pkcs12");
									...
									if (object instanceof byte[] && !force) {
										...
										if (chain != null && chain.length > 0) {
											...
											if (x509Certificate.getNotAfter().after(new Date(new Date().getTime() + 1000l*60*60*24*6))) {
												...
												artifact.releaseAllSubscriptions(virtualHost.getId(), (byte []) object);
											}
											else {
									...
									if (refresh) {
										...
										try {
											...
											map.put(virtualHost.getId() + "-pkcs12", pkcs12);
											...
											artifact.releaseAllSubscriptions(virtualHost.getId(), pkcs12);
										}
									}
								}
								finally {
						...
						logger.error("Could not finalize acme for " + virtualHost.getId(), e);
					}
				}
			}
